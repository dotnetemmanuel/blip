...
Never pass ` + "`--env`" + `. Before any POST, PUT, PATCH or DELETE, run the command with
` + "`--dry-run`" + ` and read what it would send, then run it again with ` + "`--yes`" + `.
...
func newInitCommand(rt *Runtime) *cobra.Command {
  ...
  blip init https://localhost:5001 --name orders --no-claude`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if withClaude {
				rt.writeClaudeNote(root)
				rt.writePermission(root)
			}
...
func (rt *Runtime) probeSpec(cmd *cobra.Command, base *url.URL) string {
	...
	found, ok := spec.Probe(cmd.Context(), client, base)
	if !ok {
...
func renderConfig(name, envName, authName string, base *url.URL, specURL string) string {
	...
	if config.IsLocalHost(base.Hostname()) {
...
// twice.
func (rt *Runtime) writeClaudeNote(root string) {
	path := filepath.Join(root, "CLAUDE.md")

	existing, err := os.ReadFile(path)
	switch {
	case err == nil && strings.Contains(string(existing), "blip"):
		fmt.Fprintf(rt.Stdout, "%s already mentions blip, left alone\n", path)
		return
	case err == nil:
		...
		fmt.Fprintf(rt.Stdout, "appended to %s\n", path)
	default:
		if err := os.WriteFile(path, []byte(claudeNote), 0o644); err != nil {
			rt.Warnf("%v", err)
			return
		}
		fmt.Fprintf(rt.Stdout, "wrote %s\n", path)
...
func (rt *Runtime) writePermission(root string) {
	path := filepath.Join(root, ".claude", "settings.json")

	if existing, err := os.ReadFile(path); err == nil {
		if strings.Contains(string(existing), PermissionRule) {
			fmt.Fprintf(rt.Stdout, "%s already allows blip\n", path)
			return
		}
		fmt.Fprintf(rt.Stdout, "%s exists; add %q to permissions.allow yourself\n", path, PermissionRule)
		return
	}
